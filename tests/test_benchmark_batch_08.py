"""Registration-only integrity tests for the untouched Batch 08 holdout."""
...
PRE_REGISTRATION_COMMIT = "b9c2956"
...
def _raw_field(section: str, label: str, next_label: str | None = None) -> str:
    marker = rf"(?:\*\*)?{label}:(?:\*\*)?\s*\n"
    start = re.search(marker, section).end()
...
def _parse_raw() -> list[dict[str, str]]:
    ...
    for index, heading in enumerate(headings):
        ...
        body = re.sub(
            r"\n\n---\s*$", "", _raw_field(section, "محتوى الخبر")
        ).strip()
        cases.append({
            "id": f"{70 + int(heading.group(1)):03d}",
            "url": url,
...
def test_manifest_registers_frozen_untouched_holdout() -> None:
    manifest = _json("manifest.json")
    assert manifest["batch_id"] == "batch_08"
...
def test_batch_contains_registration_and_authorized_sanitized_outputs_only() -> None:
    files = {
    ...
    assert files == {
        ...
        "execution_artifact_failure_analysis.json",
        "execution_artifact_failure_analysis.md",
        *(f"{case_id}/source.md" for case_id in EXPECTED_IDS),
    }


def test_registration_contains_no_machine_or_evaluation_outputs() -> None:
    ...
    forbidden = (
        ...
        "semantic_evidence", "candidate_assessment", "gate_decision",
        "request_builder", "provider_response", "openai_response",
        "resolver_output", "validation_output",
    )
...
def test_registration_test_imports_no_execution_modules() -> None:
    ...
    forbidden = (
        ...
        "gate", "openai", "provider", "request_builder", "workflow",
    )
...
def test_previous_benchmark_batches_are_unmodified_by_registration() -> None:
    ...
    assert not any(
        path.startswith(tuple(f"benchmark/batch_{value:02d}/" for value in range(1, 8)))
        for path in changed
    )
