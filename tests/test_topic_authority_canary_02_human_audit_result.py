...
def test_canary_002_incorrect(): assert by_id()["CANARY2-002"]["human_correctness"] == "INCORRECT_OVERRIDE"
def test_canary_002_expected_world(): assert by_id()["CANARY2-002"]["human_expected_topic"] == "WORLD"
def test_canary_002_wrong_to_wrong(): assert by_id()["CANARY2-002"]["override_transition"] == "WRONG_TO_WRONG"
...
def test_no_duplicate_audits(): assert result()["audited_override_count"] == len({item["audit_identity"] for item in result()["records"]}) == 2
...
def test_precision_one_half(): assert result()["override_precision"] == 0.5
...
def test_wrong_to_wrong_count_one(): assert result()["wrong_to_wrong_override_count"] == 1
...
def test_canary_01_history_unchanged():
    ...
    assert cumulative["audited_incorrect_override_count"] - result()["incorrect_count"] == 1
def test_cumulative_audited_count(): assert result()["cumulative_pilot_audit"]["audited_override_count"] == 5
def test_cumulative_correct_count(): assert result()["cumulative_pilot_audit"]["audited_correct_override_count"] == 3
def test_cumulative_incorrect_count(): assert result()["cumulative_pilot_audit"]["audited_incorrect_override_count"] == 2
def test_cumulative_precision(): assert result()["cumulative_pilot_audit"]["override_precision"] == 0.6
