...
MANIFEST = ROOT / "canary_sources/topic_authority_canary_01_manifest.json"
OUTPUT_JSON = ROOT / "benchmark/internal_canary/topic_authority_canary_01.json"
OUTPUT_MD = ROOT / "benchmark/internal_canary/topic_authority_canary_01.md"
SOURCE_SHA256 = "fc1edf27beb103b726a3f879b5bff9ae0e801b8b5114abc7b197e2ecac2f4a7d"
CASE_IDS = tuple(f"CANARY-{number:03d}" for number in range(1, 6))
MAX_CALLS = 5
...
def _parse_cases():
    parts = re.split(r"(?=^canary_id: CANARY-\d{3}$)", SOURCE.read_text(encoding="utf-8"), flags=re.MULTILINE)
    cases = []
    for part in (item for item in parts if item.strip()):
        case_id = re.search(r"^canary_id: (CANARY-\d{3})$", part, re.MULTILINE).group(1)
...
def _summary(cases, provider_calls, stopped, contract_violations, global_before, global_after):
    ...
    unsafe = bool(contract_violations) or any(item["route"] != "INTERNAL_TOPIC_AUTHORITY_CANARY_PATH" for item in cases)
    classification = "FIRST_CANARY_UNSAFE" if unsafe else "FIRST_CANARY_STOPPED_SAFELY" if stopped else "FIRST_CANARY_OPERATIONALLY_CLEAN" if consumed else "FIRST_CANARY_CLEAN_WITH_NO_AUTHORITY_OPPORTUNITY"
    return {
        "canary_id": "topic_authority_canary_01",
        ...
        "retry_attempts": 0,
        "resolver_adjudicated_accepted": sum(item["authority_source"] == "ADJUDICATION" or item["authority_applied"] for item in cases),
        ...
        "audit_queue": [{"canary_id": item["canary_id"], "decision_fingerprint": item["input_fingerprint"], "correctness": None} for item in cases if item["authority_consumed"]],
...
def render_markdown(summary):
    return f"""# First Internal Topic Authority Canary
...
def run_canary(*, model="gpt-5-mini", provider=None, output_json=OUTPUT_JSON, output_md=OUTPUT_MD, monotonic=time.monotonic):
    ...
    entry = InternalTopicAuthorityCanaryEntrypoint(workflow, TopicAuthorityCanaryRouteConfig(True, "HKEI-212"))
    ...
    for case_id, fields in _parse_cases():
        ...
        records.append(_case_record(case_id, safe, raw, capturing, latency))
        if raw.contract_violations:
