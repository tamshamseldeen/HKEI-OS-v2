...
OUTPUT_JSON = ROOT / "benchmark/internal_canary/topic_authority_canary_03.json"
OUTPUT_MD = ROOT / "benchmark/internal_canary/topic_authority_canary_03.md"
SOURCE_SHA256 = "fb711ba1d0c3a4a8cd32d9f8ea2729f64754d0f38db4560a099ee23ffe9ca531"
CASE_IDS = tuple(f"CANARY3-{number:03d}" for number in range(1, 6))
MAX_CALLS = 5
...
def _verify_source():
    ...
    if any(data[name] != 0 for name in (
        "duplicates_with_canary_01",
        "duplicates_with_benchmark_batches",
        "duplicates_with_HKEI_216_fixtures",
    )):
        raise RuntimeError("registered source freshness audit failed")


def _parse_cases():
    parts = re.split(r"(?=^canary_id: CANARY3-\d{3}$)", SOURCE.read_text(encoding="utf-8"), flags=re.MULTILINE)
    cases = []
    for part in (item for item in parts if item.strip()):
        case_id = re.search(r"^canary_id: (CANARY3-\d{3})$", part, re.MULTILINE).group(1)
...
def _summary(cases, provider_calls, stopped, contract_violations, global_before, global_after):
    ...
    classification = "THIRD_CANARY_UNSAFE" if unsafe else "THIRD_CANARY_STOPPED_SAFELY" if stopped else "THIRD_CANARY_OPERATIONALLY_CLEAN" if consumed else "THIRD_CANARY_CLEAN_WITH_NO_AUTHORITY_OPPORTUNITY"
    return {
        "canary_id": "topic_authority_canary_03",
...
def render_markdown(summary):
    return f"""# Third Internal Topic Authority Canary
...
def run_canary(*, model="gpt-5-mini", provider=None, output_json=OUTPUT_JSON, output_md=OUTPUT_MD, monotonic=time.monotonic):
    ...
    if output_json == OUTPUT_JSON and (OUTPUT_JSON.exists() or OUTPUT_MD.exists()):
        raise RuntimeError("Canary 03 evaluation artifact already exists")
    ...
    entry = InternalTopicAuthorityCanaryEntrypoint(workflow, TopicAuthorityCanaryRouteConfig(True, "HKEI-225"))
