"""Run the advanced editorial holdout validation against Batch 05."""
...
def analyze_validation(
    *,
    ...
    """Evaluate every Batch 05 case using only the registered editorial labels."""
        ...
        cases.append(
            {
                ...
                "contextual_item_count": len(
                    result.contextual_evidence.all_items
                ),
                ...
                "contextual_suppression_labels": _unique_labels(
                    label
                    for item in result.contextual_evidence.all_items
                    for label in item.suppresses
                ),
                "semantic_relationship_count": len(semantic.relationships),
                "semantic_relationship_types": _unique_labels(
    ...
    return {
        ...
        "cases_with_primary_semantic_domain": sum(
            bool(case["semantic_primary_domain_candidates"]) for case in cases
        ),
    ...
    for case in cases:
        ...
        if dimension == "topic":
            fields.extend(
                (
                    ...
                    ("Contextual Supports", _display(case["contextual_support_labels"])),
        ...
        fields.extend(
            (
                ...
                (
                    "Supporting Signals",
                    _display(case[f"{evidence_prefix}_supporting_signals"]),
                ),
...
def render_markdown(analysis: dict[str, Any]) -> str:
    """Render the required deterministic Batch 05 Markdown report."""
    lines = [
    ...
    lines.extend(
        (
            ...
            "Cases With Primary Semantic Domain:", str(analysis["cases_with_primary_semantic_domain"]), "",
...
def render_console(analysis: dict[str, Any]) -> str:
    ...
    lines = [
        "=== BATCH 05 ADVANCED-RISK EDITORIAL HOLDOUT ===", "",
        ...
        "Full Case Accuracy:", f'{analysis["full_case_accuracy"]:.2f}%', "",
        "Fully Matched:", str(analysis["fully_matched_cases"]), "",
        "Contextual Evidence Cases:", str(analysis["cases_with_contextual_evidence"]), "",
        "Semantic Relationship Cases:", str(analysis["cases_with_semantic_relationships"]), "",
        "Primary Semantic Domain Cases:", str(analysis["cases_with_primary_semantic_domain"]), "",
        "Semantic Evidence Used:", str(analysis["semantic_evidence_used"]), "",
    ]
    lines.extend(
