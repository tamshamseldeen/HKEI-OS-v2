"""Aggregate HKEI-214 human audits and execute the canonical pilot stop policy."""
...
OUTPUT_JSON = ROOT / "benchmark/internal_canary/topic_authority_canary_01_human_audit_result.json"
OUTPUT_MD = ROOT / "benchmark/internal_canary/topic_authority_canary_01_human_audit_result.md"
HISTORICAL_CANARY_SHA256 = "145d6de20e2764f4c01b74b54274b7f758a8e40340183b3f0b8be2b7222e2295"
...
def _post_stop_proxy(stop_decision):
    ...
    safe = OperationalTopicAuthorityCanary(runtime, InMemoryTopicAuthorityObservationSink()).execute(
        ...
        __import__("src.resolution", fromlist=["TopicAuthorityConsumerRoute"]).TopicAuthorityConsumerRoute.INTERNAL_TOPIC_AUTHORITY_CANARY_PATH,
    )
...
def build_result():
    ...
    operational = TopicAuthorityMetrics(
        articles_processed=5, topic_adjudication_requested=5, provider_calls=5,
        valid_adjudications=5, resolver_adjudicated_accepted=3,
        authoritative_topic_overrides=3, deterministic_topic_preserved=2,
    )
    ...
    per_case = [{key:item[key] for key in (
        ...
        "human_expected_topic", "human_correctness", "review_timestamp", "reviewer_notes",
    )} for item in packet["records"]]
    return {
        "audit_id": "topic_authority_canary_01_human_audit_result",
        ...
        "audit_outcome_classification": "FIRST_CANARY_AUDIT_MIXED",
        "next_recommended_step": "ANALYZE_FIRST_CANARY_WRONG_OVERRIDE_ONCE",
        "provider_calls": 0,
...
def render_markdown(result):
    return f"""# First Canary Human Audit Result
