"""Run semantic-aware full editorial validation against Batch 03."""
...
def analyze_validation(
    *,
    ...
    """Evaluate all registered cases through the experimental workflow."""
...
def render_json(analysis: dict[str, Any]) -> str:
    """Render deterministic semantic validation JSON without source text."""
    return json.dumps(analysis, ensure_ascii=False, indent=2) + "\n"


def _display(values: list[str]) -> str:
    """Render semantic labels or explicit None."""
    return ", ".join(values) if values else "None"


def _mismatch_lines(analysis: dict[str, Any], dimension: str) -> list[str]:
    """Render raw mismatch evidence for one output dimension."""
    mismatches = [
    ...
    for case in mismatches:
        for label, value in (
            ...
            ("Semantic Primary", _display(case["semantic_primary_domain_candidates"])),
            ("Semantic Format", _display(case["semantic_format_support"])),
            ("Semantic Suppression", _display(case["semantic_suppressions"])),
        ):
...
def render_markdown(analysis: dict[str, Any]) -> str:
    """Render the required table, metrics, baseline, and mismatches."""
    lines = [
    ...
    lines.extend(
        (
            ...
            str(analysis["semantic_evidence_used"]),
            "",
            "Semantic Suppression Used:",
            str(analysis["semantic_suppression_used"]),
            "",
    ...
    for title, dimension in (
        ...
        ("Intent Mismatches", "reader_intent"),
    ):
        lines.extend((f"## {title}", ""))
        lines.extend(_mismatch_lines(analysis, dimension))
    return "\n".join(lines)


def render_console(analysis: dict[str, Any]) -> str:
    """Render concise semantic-aware validation metrics and case results."""
    lines = [
        "=== BATCH 03 SEMANTIC-AWARE FULL VALIDATION ===",
        "",
        "Cases:",
...
def main() -> int:
    """Run validation, persist reports, and return registered status."""
    analysis = analyze_validation()
    (BATCH_ROOT / "semantic_full_validation.json").write_text(
        render_json(analysis), encoding="utf-8"
    )
    (BATCH_ROOT / "semantic_full_validation.md").write_text(
